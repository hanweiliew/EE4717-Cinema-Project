...
if($quanSeats > 0 ){
	...
	updateOrderTable($quanSeats,1,$quanSeats*13.5);
	foreach ($selectedSeats as $seat){	
...
function updateSeatTable($orderid,$seats,$ticket_id ) {
	...
	$sql = "INSERT INTO f32ee.cinema1 (order_id,ticket_id, ticket_seat) VALUES ($orderid,$ticket_id,'$seats');";
	if (!mysqli_query($conn, $sql)) {
		echo "Failed to update database: " . mysqli_error($conn);
		mysqli_close($conn);
	}
...
<body>
	...
	<form action="finalconfirmation1.php" method="post" class="finalconfirm">
